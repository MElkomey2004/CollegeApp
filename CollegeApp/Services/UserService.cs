...
namespace CollegeApp.Services
{
	public class UserService : IUserService
	{
		...
		public async Task<bool> CreateUserAsync(UserDTO dto)
		{
			...
			var existingUser =  _userRepository.Get(u => u.UserName.Equals(dto.Username));
		...
		{
			var users =  _userRepository.GetAllByFilter(u => !u.IsDeleted);
		...
		{
			var user =  _userRepository.Get(u => !u.IsDeleted && u.Id == id);
		...
		{
			var user =  _userRepository.Get(u => !u.IsDeleted && u.UserName.Equals(username));
		...
		public async Task<bool> UpdateUserAsync(UserDTO dto)
		{
			...
			var existingUser =  _userRepository.Get(u => !u.IsDeleted && u.Id == dto.Id, true);
			if (existingUser == null)
			{
			...
			userToUpdate.ModifiedData = DateTime.Now;
		...
		public async Task<bool> DeleteUser(int userId)
		{
			...
			var existingUser =  _userRepository.Get(u => !u.IsDeleted && u.Id == userId, true);
			if (existingUser == null)
			{
